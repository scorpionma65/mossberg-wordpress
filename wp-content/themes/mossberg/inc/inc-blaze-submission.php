...
if(isset($_POST['submit'])) {
	// Check	
	$select_a_blaze_magazine_type = sanitize_text_field($_POST['select_a_blaze_magazine_type']);
	if(!$select_a_blaze_magazine_type) {
		$message .= "<p class=\"form_message_fail\">Please select a Magazine Type</p>";
	}
	...
	if($select_a_blaze_magazine_type && $firstname && $lastname && $email && $address && $city && $state && $zip) {	
	
		// Online
		$args = array('name'=>'blaze-offer-redeem-online', 'post_type'=>'post', 'post_status'=>'publish', 'numberposts'=>1);
		...
		// HubSpot API
		include(ABSPATH.'wp-content/themes/mossberg/inc/hubspot/hubspot-blaze-submission.php');
		$show = FALSE;
		$mail = TRUE;
	}
}
...
// Form
if($show) {
	include(TEMPLATEPATH.'/inc/forms/form-blaze-submission.php');
}
?>
