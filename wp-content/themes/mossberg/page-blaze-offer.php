...
<!-- Posts -->
<!-- Submission -->
<div class="blaze_submission">
<?php include(TEMPLATEPATH.'/inc/inc-blaze-submission.php');?>
</div>
<!-- Submission -->
<!-- Mail Option -->
<?php
...
if($mail) {
	...
	$zip = urlencode($zip);
	$magazine = urlencode($select_a_blaze_magazine_type);
	echo "<div class=\"post_text\">
	<h4>Having trouble redeeming online? <a href=\"".get_bloginfo('home')."/blazeoffer-mail?firstname=$firstname&lastname=$lastname&email=$email&address=$address&city=$city&state=$state&zip=$zip&magazine=$magazine\" target=\"_blank\">Print the Mail-in Form to Redeem by Mail &raquo;</a></h4>
	</div>";
}
?>
<!-- Mail Option -->
...
<!-- Offer Sidebar -->
<?php
$args = array('name'=>'blaze-offer-sidebar', 'post_type'=>'post', 'post_status'=>'publish', 'numberposts'=>1);
$posts = get_posts($args);
if($posts) { 
	$post_title = $posts[0]->post_title;
	$post_content = wpautop($posts[0]->post_content);
