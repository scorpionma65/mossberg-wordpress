<?php
// Message
$message = "<p>Enter your Username/Password to add selected items to your cart. Don't have an account? <a href=\"javascript:void(0);\" onclick=\"switch_form();\">Register Now &raquo;</a></p>";
$show = TRUE;
...
if(isset($_POST['submit_cart_register'])) {
	...
	if(!empty($_POST['sku_list'])) {
		...
		$sku_list = FALSE;
		$message .= "<p>There are no Selected Parts to add</p>";
	}
	...
	if($first_name && $last_name && $email && $password && $sku_list) {
...
if(isset($_POST['submit_cart_login'])) {
	...
	if($username && $password && $sku_list) {
		//echo "<p>2. AUTH: $username / $password / $sku_list</p>";
		// Token
		include(TEMPLATEPATH.'/inc/magento/magento-api-signature.php');
...
if($token) {
	...
	foreach($skus as $key => $sku) {
		...
		if($item) {
			...
			$message .= "<p>$sku could not be added to cart. This item is out of stock.</p>";
		}				
	}
	if($items) {
		switch($success) {
			case 'R':
			$message .= "<h3>Account Created and Item(s) Added. <a href=\"".get_bloginfo('url')."/store/customer/account/login/\">Log In to View Cart &raquo;</a></h3>";
			break;
			case 'L':
			$message .= "<h3>Item(s) Added. <a href=\"".get_bloginfo('url')."/store/customer/account/login/\">Log In to View Cart &raquo;</a></h3>";
			break;
		}
		$show = FALSE;
	}	
...
if($show) {
	include(TEMPLATEPATH.'/inc/magento/magento-schematic-cart-form.php');
}
?>